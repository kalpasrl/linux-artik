...
static void max31913_parityCheck( struct max31913 *mc )
{
...
static int max31913_probe(struct spi_device *spi)
{
	...
	int ret;
        ...
        dev_dbg(&spi->dev, "max31913->plat_data \n") ;
	pdata = dev_get_platdata(&spi->dev);
        dev_dbg(&spi->dev, "max31913->probe \n") ;

        match = of_match_device(of_match_ptr(max31913_spi_of_match), &spi->dev);
	if (match) {

            dev_dbg(&spi->dev, "max31913->match  \n" ) ;
            ...
            status = of_property_read_u32(spi->dev.of_node,
             ...
             dev_dbg(&spi->dev, "max31913->frequency : %d - cs-gpio : %d - \n" ,spi_max_frequency,cs_gpio) ;
             dev_dbg(&spi->dev, "max31913->base gpio : %d - \n" ,base_gpio) ;
         }
         else
         {
             dev_dbg(&spi->dev, "max31913->no match  \n" ) ;
	...
	if (ret < 0)
		return ret;
        dev_dbg(&spi->dev, "max31913->setup ok \n" ) ;
	...
	if (ret) {
        ...
        dev_dbg(&spi->dev, "max31913->read ok completed %d .. \n", mc->port_status ) ;
...
static int max31913_remove(struct spi_device *spi)
{
	struct max31913 *mc;
        ...
        dev_dbg(&spi->dev, "max31913->remove") ;
