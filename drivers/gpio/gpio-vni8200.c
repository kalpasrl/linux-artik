...
static void vni8200_parityCheck( struct vni8200 *mc )
{
...
static int vni8200_probe(struct spi_device *spi)
{
	...
	int ret;
        ...
        dev_dbg(&spi->dev, "vni8200->plat_data \n") ;
	pdata = dev_get_platdata(&spi->dev);
        dev_dbg(&spi->dev, "vni8200->probe \n") ;

        match = of_match_device(of_match_ptr(vni8200_spi_of_match), &spi->dev);
	if (match) {

            dev_dbg(&spi->dev, "vni8200->match  \n" ) ;
            ...
            status = of_property_read_u32(spi->dev.of_node,
             ...
             dev_dbg(&spi->dev, "vni8200->frequency : %d - cs-gpio : %d - \n" ,spi_max_frequency,cs_gpio) ;
             dev_dbg(&spi->dev, "vni8200->base gpio : %d - \n" ,base_gpio) ;
         }
         else
         {
             dev_dbg(&spi->dev, "vni8200->no match  \n" ) ;
	...
	if (ret)
		return ret;
        dev_dbg(&spi->dev, "vni8200->read ok completed %d .. \n", mc->port_status ) ;
        dev_dbg(&spi->dev, "vni8200->port config %d ..\n", mc->port_config) ;
...
static int vni8200_remove(struct spi_device *spi)
{
	struct vni8200 *mc;
        ...
        dev_dbg(&spi->dev, "vni8200->remove") ;
